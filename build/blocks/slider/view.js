 ...
 * Helpers
 */
const q = (root, selector) => root.querySelector(`:scope ${selector}`) || root.querySelector(selector);
...
const buildVerticalOptions = (ref, baseOptions) => {
  ...
  const buildTextNav = swiper => {
    ...
    const realSlides = Array.from(swiper.el.querySelectorAll('.swiper-slide')).filter(s => !s.classList.contains('swiper-slide-duplicate'));
    ...
    const buttons = Array.from(navRoot.querySelectorAll('.swiper-text-nav__btn'));
    const setActive = () => {
  ...
  const updateUI = swiper => {
    ...
    if (progressBar) {
      ...
      for (let i = 0; i < totalSlides; i++) {
        ...
        dot.className = 'swiper-progress-bar__dot' + (i === current ? ' is-active' : '');
...
(0,_wordpress_interactivity__WEBPACK_IMPORTED_MODULE_0__.store)('abtion-block-library/slider', {
  callbacks: {
    setup() {
      const {
        ref
