...
class AccordionItem {
  ...
  getAncestors() {
    ...
    while (current?.parentNode && current.parentNode !== document.documentElement) {
      ...
      if (parent instanceof HTMLElement && parent.dataset?.accordionItem === 'true') {
        list.push(parent);
      }
      current = parent;
    }
  ...
  getSiblings() {
    ...
    ['previous', 'next'].forEach(dir => {
      ...
      while (element) {
        ...
        if (sib && sib instanceof HTMLElement && sib.dataset?.accordionItem === 'true') {
          element = sib;
